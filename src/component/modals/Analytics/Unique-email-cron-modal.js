import React, {
  ...
  const AnalyticEmailbox = ({
    ...
    let cio =(ind>-1 ? filtered[ind]['value'].split(',').map((e)=>{return e.charAt(0).toUpperCase() + e.slice(1)}) : '');
    let defaultaccounts =
      page == "freshdata"
    ...
    const fetchlist = async (type) => {
      let data = await Fetchdata.fetchanalytictemplate({ type: type,'posttype':'fetchanalytictemplate','user':auth.userid }).then(
        (response) => {
          return response;
        }
      );
      ...
      <>
        ...
        >
                    ...
                    <TableVirtuoso
                      ...
                      itemContent={(index, user) => (
                        <>
                          ...
                          <td className="column-value">{user[0]} {user[1]}</td>
          ...
          />
                    ...
                    <select onChange={(e)=>(changeuser(e))}  id="templateid" className="form-select">
