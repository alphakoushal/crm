...
const Datanalyticsidebar = ({ data, showanalyticsidebar }) => {
  ...
  const statusCounts = data.reduce(
    (acc, item) => {
      const e_m = analyticemailstatus[item[18]] ?? "";
      const c_l = analyticcallstatus[item[20]] ?? "";
      if (e_m && e_m.trim() !== "") {
    ...
    try {
      ...
      if (type == "email") {
        countryCounts = data.reduce((acc, code) => {
          const country =
            code[13] && code[18] !== "" && (v == "All" || v == code[18])
              ? defaultvalue.countriescode[code[13]]?.name ?? code[13]
              : "";
        ...
        countryCounts = data.reduce((acc, code) => {
          const countryCode = defaultvalue.phoneextension(code[10]+'', code[13]);
          const country =
            code[13] && code[20] !== "" && (v == "All" || v == code[20])
  ...
  return (
    ...
    xaxis: {
      categories: Object.keys(analyticdata?.sentemail?.account ?? {}),
    },
    plotOptions: {
      bar: {
        horizontal: false,
    ...
    tooltip: {
      custom: ({ series, seriesIndex, dataPointIndex, w }) => {
        const accounts = Object.values(analyticdata?.sentemail?.account ?? {});
        const accountKeys = Object.keys(analyticdata?.sentemail?.account ?? {});
        const entry = accounts[dataPointIndex];
        
        const delivered = entry.success;
        const pending = entry.pending;
        const totalCrons = entry.completed_crons + entry.pending_crons;
        const totalEmails = entry.total;
        ...
        return `
          <div class="apexcharts-tooltip-title" style="padding:6px 10px;">
            ${accountKeys[dataPointIndex]}
          </div>
          <div class="apexcharts-tooltip-series-group" style="padding:6px 10px; display:flex; flex-direction:column;">
          <div class='col-12'>${getColorDot('#FEB019')} Total Emails: <strong>${totalEmails}</strong></div>
          <div class='col-12'>${getColorDot('#775DD0')} Total Crons: <strong>${totalCrons}</strong></div>
          <div class='col-12'>${getColorDot('#FF4560')} Pending Emails: <strong>${pending}</strong></div>
            <div class='col-12'>${getColorDot('#00E396')} Delivered Emails: <strong>${delivered}</strong></div>
          </div>
        `;
      },
    },
    ...
    colors: ['#FEB019', '#775DD0','#FF4560','#00E396'],
    legend: {
      position: "top",
    },
    yaxis: {
      // optional: logarithmic: true
      min: 0,
    },
  }}
                      series={[
                        {
                          ...
                          ).map((value, index) => value.completed_crons + value.pending_crons),
                        },{
                          name: "Total Emails",
                          hidden: true,
                          data: Object.values(
