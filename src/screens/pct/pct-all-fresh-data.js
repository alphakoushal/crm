...
async function pickvalue(e,i,ni,key)
{
  e.stopPropagation();
    if(e.detail==1)
    {
    document.querySelector('.cell-name').value=key?.key??document.querySelectorAll('.custom-table table thead tr+tr th')[ni].querySelector('.headers').innerText;
...
async function editinfo(state,app='',index)
{
    if(state)
    {
const getdata=await Fetchdata.pctaxiosrequest({"publication_value":app,'posttype':'freshdataedit','type':"editdata"}).then((response)=>{return response});
updateeditmodal({state:state,data:getdata.data.data,index:index});
    }
    else
    {
...
{
  let found=false;
  const index_of_data = agentdata.filter((e)=>{return e.name==data});
  return index_of_data.length>0 ? true : false;
}
const pagination = (data) => {
  let previosindex=0;let nextindex=0;
  previosindex =(data.action=='previous' && data.index==0 ? 0 : data.index-1)
...
{showeditmodal.state==true ? <Editmodal pagination={pagination} lawfirmdata={lawfirmdata} agentdata={agentdata} alldata={d2} changedata={changedata} show={showeditmodal} fn={editinfo}></Editmodal> : <></> }
    <Commentmodal/>
    <Style></Style>
    <IpoperationHeader platform={platform} changedata={changedata}  except={true} completedata={d2} alldata={d} showmailbox={showmailbox} showdupemailbox={showdupemailbox} showcronbox={()=>{return false;}}  clearfilters={clearfilter} refreshdata={loaddata} formdatas={formdata} showcurrencies={()=>{return false;}}></IpoperationHeader>
...
<th>
 ...
 <th style={{background: 'white', position: 'sticky', left: 0, zindex: 1 }}><div  className="headers">APPLN.NO. <i className="ti ti-sort-ascending" onClick={(e)=>{sortdata(e,2)}}></i></div><input className="filter" onKeyUp={(e)=>filterdata(2,e.target.value)} type='text'></input></th>
...
<th style={{  background: 'white' }}><div className="headers">Submitted on<i className="ti ti-sort-ascending" onClick={(e)=>{sortdata(e,56)}}></i> </div><input className="filter" onKeyUp={(e)=>filterdata(56,e.target.value)} type='text'></input></th>
<th style={{  background: 'white' }}><div className="headers">Operation Team<i className="ti ti-sort-ascending" onClick={(e)=>{sortdata(e,57)}}></i> </div><input className="filter" onKeyUp={(e)=>filterdata(57,e.target.value)} type='text'></input></th>
...
<th style={{  background: 'white' }}><div className="headers">Filename<i className="ti ti-sort-ascending" onClick={(e)=>{sortdata(e,64)}}></i> </div><input className="filter" onKeyUp={(e)=>filterdata(64,e.target.value)} type='text'></input></th>
<th style={{  background: 'white' }}><div className="headers">Remarks<i className="ti ti-sort-ascending" onClick={(e)=>{sortdata(e,64)}}></i> </div><input className="filter" onKeyUp={(e)=>filterdata(64,e.target.value)} type='text'></input></th>
      ...
      itemContent={(index, user) => (
        <>
        <td>{index}</td>
<td onClick={(e)=>{pickvalue(e,2,0,{key:'APPLN.NO.'})}} className="column-value" style={{ background:user[60], position: 'sticky', left: 0, zIndex: 1 }}><input className='appno' value={user[2]} onClick={(event)=>pushdata(event,user[2])} style={{'position':"absolute",'top':'18px','left':'0'}} type='checkbox'></input><a target="blank" href={"https://patentscope.wipo.int/search/en/detail.jsf?docId="+user[0]}>{user[2]}</a><i onClick={()=>{editinfo(true,user[2],index)}} style={{'position': 'absolute','top': '1px','right': '5px','background': '#5d87ff','width': '14px','height': '14px','display': 'flex','lineHeight': '14px','borderRadius': '50%','color': 'white','justifyContent': 'center'}} className="ti ti-edit"></i></td>
<td onClick={(e)=>{pickvalue(e,1,1,{key:'Title'})}} className="column-value" style={{  }}>{user[1]}</td>
<td onClick={(e)=>{pickvalue(e,3,2,{key:'COUNTRY'})}} className="column-value small" style={{  }}>{user[3]}</td>
<td onClick={(e)=>{pickvalue(e,4,3,{key:'PRIOTITY DATE'})}} className="column-value" style={{  }}>{user[4]}</td>
<td onClick={(e)=>{pickvalue(e,5,4,{key:'DEADLINE - 30 mth'})}} className="column-value" style={{  }}>{user[5]}</td>
<td onClick={(e)=>{pickvalue(e,6,5,{key:'DEADLINE - 31 mth'})}} className="column-value" style={{  }}>{user[6]}</td>
<td onClick={(e)=>{pickvalue(e,7,6,{key:'APPLICANT NAME'})}} className="APPLICANT NAME" style={{  }}>{user[7]} {(checkiffound(user[7]) ? <span class="badge fw-semibold py-1 bg-primary-subtle text-primary">Found</span> : '')}</td>
<td onClick={(e)=>{pickvalue(e,53,7,{key:'Unique/Dupe'})}} className="column-value" style={{  }}>{user[53]}</td>
<td onClick={(e)=>{pickvalue(e,54,8,{key:'Gen/Non Gen'})}} className="column-value" style={{  }}>{user[54]}</td>
<td onClick={(e)=>{pickvalue(e,8,9,{key:'Applicant Status'})}} className="column-value" style={{  }}>{user[8]}</td>
<td onClick={(e)=>{pickvalue(e,9,10,{key:'CONTACT INFO OF'})}} className="column-value" style={{  }}>{user[9]}</td> 
<td onClick={(e)=>{pickvalue(e,10,11,{key:'CONTACT PERSON'})}} className="column-value" style={{  }}>{user[10]}</td>
<td  onClick={(e)=>{pickvalue(e,11,12,{key:'EMAIL ID'})}} className={`cursor-pointer text-primary column-value align-items-center ${(tablesetting.countred(user[11],11,d) ? 'red-dupe' : '')}`} style={{  }}><i className="ti ti-refresh rotate hide profilefetch"></i><span className="email-id">{user[11]}</span></td>
<td  onClick={(e)=>{pickvalue(e,12,13,{key:'Domain'})}} className="column-value" style={{  }}>{user[12]}</td>
<td  onClick={(e)=>{pickvalue(e,13,14,{key:'PH. NO.'})}} className="column-value" style={{  }}>{user[13]}</td>
<td  onClick={(e)=>{pickvalue(e,14,15,{key:'Pages'})}} className="column-value small" style={{  }}>{user[14]}</td>
<td  onClick={(e)=>{pickvalue(e,15,16,{key:'Claim'})}} className="column-value small" style={{  }}>{user[15]}</td>
<td  onClick={(e)=>{pickvalue(e,16,17,{key:'Priority'})}} className="column-value small" style={{  }}>{user[16]}</td> 
<td  onClick={(e)=>{pickvalue(e,17,18,{key:'Drawings'})}} className="column-value small" style={{  }}>{user[17]}</td>
<td  onClick={(e)=>{pickvalue(e,18,19,{key:'ISR'})}} className="column-value small" style={{  }}>{user[18]}</td> 
<td  onClick={(e)=>{pickvalue(e,19,20,{key:'REF. NO.'})}} className="column-value" style={{  }}>{user[19]}</td>
<td  onClick={(e)=>{pickvalue(e,20,21,{key:'First Email Date'})}} className="column-value" style={{  }}>{user[20]}</td>
<td  onClick={(e)=>{pickvalue(e,21,22,{key:'FollowUp date'})}} className="column-value" style={{  }}>{user[21]}</td>
<td  onClick={(e)=>{pickvalue(e,22,23,{key:'Next Follow Up'})}} className="column-value" style={{  }}>{user[22]}</td>
<td  onClick={(e)=>{pickvalue(e,23,24,{key:'Previous Status'})}} className="column-value" style={{  }}>{user[52]}</td>
<td  onClick={(e)=>{pickvalue(e,23,25,{key:'Email Status'})}} className="column-value" style={{  }}>{emailstatus[user[23]] ?? ''}</td>
<td  onClick={(e)=>{pickvalue(e,24,26,{key:'Call Status'})}} className="column-value" style={{  }}>{callstatus[user[24]] ?? ''}</td>
<td onClick={(e)=>{pickvalue(e,25,27,{key:'Comment'})}} className="column-value" style={{  }} dangerouslySetInnerHTML={{__html: removeduplicate(user[25])}} />
<td  onClick={(e)=>{pickvalue(e,26,28,{key:'Agent name'})}} className="column-value" style={{  }}>{user[26]}</td>
<td  onClick={(e)=>{pickvalue(e,27,29,{key:'Agent Email Id'})}} className={`column-value ${(tablesetting.countred(user[27],27,d) ? 'red-dupe' : '')}`} style={{  }}>{user[27]}</td>
<td  onClick={(e)=>{pickvalue(e,28,30,{key:'Agent Domain'})}} className="column-value" style={{  }}>{user[28]}</td>
<td  onClick={(e)=>{pickvalue(e,29,31,{key:'Agent Phone'})}} className="column-value" style={{  }}>{user[29]}</td>
<td  onClick={(e)=>{pickvalue(e,30,32,{key:'Previous Email Status'})}} className="column-value" style={{  }}>{user[30]}</td>
<td  onClick={(e)=>{pickvalue(e,31,33,{key:'Company'})}} className="column-value" style={{  }}>{user[31]}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,32,34,{key:'IN'})}}  style={{  }}>{costs.IN.apply({'appno':user[2],'c':'IN','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,33,35,{key:'CA'})}}  style={{  }}>{costs.CA.apply({'appno':user[2],'c':'CA','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,34,36,{key:'CN'})}}  style={{  }}>{costs.CN.apply({'appno':user[2],'c':'CN','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,35,37,{key:'JP'})}}  style={{  }}>{costs.JP.apply({'appno':user[2],'c':'JP','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,36,38,{key:'AU'})}}  style={{  }}>{costs.AU.apply({'appno':user[2],'c':'AU','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,37,39,{key:'BR'})}}  style={{  }}>{costs.BR.apply({'appno':user[2],'c':'BR','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,38,40,{key:'US'})}}  style={{  }}>{costs.US.apply({'appno':user[2],'c':'US','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,39,41,{key:'KR'})}}  style={{  }}>{costs.KR.apply({'appno':user[2],'c':'KR','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,40,42,{key:'EP'})}}  style={{  }}>{costs.EP.apply({'appno':user[2],'c':'EP','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,41,43,{key:'RU'})}}  style={{  }}>{costs.RU.apply({'appno':user[2],'c':'RU','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,42,44,{key:'MX'})}}  style={{  }}>{costs.MX.apply({'appno':user[2],'c':'MX','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,43,45,{key:'MY'})}}  style={{  }}>{costs.MY.apply({'appno':user[2],'c':'MY','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,44,46,{key:'PH'})}}  style={{  }}>{costs.PH.apply({'appno':user[2],'c':'PH','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,45,47,{key:'TH'})}}  style={{  }}>{costs.TH.apply({'appno':user[2],'c':'TH','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,46,48,{key:'ID'})}}  style={{  }}>{costs.ID.apply({'appno':user[2],'c':'ID','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,47,49,{key:'NZ'})}}  style={{  }}>{costs.NZ.apply({'appno':user[2],'c':'NZ','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,48,50,{key:'ZA'})}}  style={{  }}>{costs.ZA.apply({'appno':user[2],'c':'ZA','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,49,51,{key:'VN'})}}  style={{  }}>{costs.VN.apply({'appno':user[2],'c':'VN','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,50,52,{key:'SG'})}}  style={{  }}>{costs.SG.apply({'appno':user[2],'c':'SG','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td className={"column-value"+(defaultdata.showcurrencytab ? '' : ' hiddencol')}  onClick={(e)=>{pickvalue(e,51,53,{key:'CO'})}}  style={{  }}>{costs.CO.apply({'appno':user[2],'c':'CO','as':user[8],'ci':user[9],'pages':user[14],'claim':user[15],'priority':user[16],'co':user[3],'isa':user[18],'standard':standard})}</td>
<td  onClick={(e)=>{pickvalue(e,52,54,{key:'Month'})}} className="column-value" style={{  }}>{user[55]}</td>
<td  onClick={(e)=>{pickvalue(e,53,55,{key:'Submitted on'})}} className="column-value" style={{  }}>{user[56]}</td>
<td  onClick={(e)=>{pickvalue(e,54,56,{key:'Operation Team'})}} className="column-value" style={{  }}>{user[57]}</td>
<td  onClick={(e)=>{pickvalue(e,55,57,{key:'Prev Assigned'})}} className="column-value" style={{  }}>{user[58]}</td>
<td  onClick={(e)=>{pickvalue(e,56,58,{key:'New Assigned'})}} className="column-value" style={{  }}>{user[63]}</td>
<td  onClick={(e)=>{pickvalue(e,57,59,{key:'Agent Unique/Dupe'})}} className="column-value" style={{  }}>{user[61]}</td>
<td  onClick={(e)=>{pickvalue(e,58,60,{key:'Agent Gen/Non Gen'})}} className="column-value" style={{  }}>{user[62]}</td>
<td  onClick={(e)=>{pickvalue(e,59,61,{key:'Assigned Value'})}} className="column-value" style={{  }}>{user[64]}</td>
<td  onClick={(e)=>{pickvalue(e,59,61,{key:'Filename'})}} className="column-value" style={{  }}>{user[65]}</td>
<td  onClick={(e)=>{pickvalue(e,59,61,{key:'Remarks'})}} className="column-value" style={{  }}>{user[66]}</td>
