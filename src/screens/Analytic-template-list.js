...
const AnalytictemplateList = () =>{
  ...
  const sendtome = async(e,id) =>{
    ...
    let res =await Uploaddata.sendtemplatetome({'email':email,'id':id,'posttype':'sendanalytictemplatetome'}).then((resposne)=>{return resposne});
...
let data=await Fetchdata.fetchanalytictemplate({'posttype':type,type:'1','user':auth.userid }).then((response)=>{ return response});
settemplate(data.data.data);
  }
  useEffect(()=>{
    document.querySelector('table').classList.add("table","table-bordered","table-hover");

    fetchlist('fetchanalytictemplate');
  },[])
    return (
        <>
      ...
      itemContent={(index, user) => (
        ...
        <>
                           <td>
                            ...
                            <td>
                              <p className="mb-0 fs-1"><Link to={"/edit-analytic-template?id="+user['id']}>Edit</Link></p>
                            </td>
                            <td>
...
export default AnalytictemplateList;